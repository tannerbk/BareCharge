...
double getVoltage(float trace_length, DataCluster *datacluster, 
                  double pedestal, double dy, int i);
...
int main (int argc, char* argv[])
{
    ...
    try{
        ...
        while (ifs.good()){
            ...
            cout << "Trace length is:        " << window_length*dx*1e9 << " ns" << endl;
            cout << "Pedestal length:        " << window_width*dx*1e9 << " ns"<< endl;
            cout << "Integration length:     " << signal_window*dx*1e9 << " ns" << endl;

            // Loop over every trace
            for (int j = 0; j < datacluster->n_traces; j++){
                ...
                double pedestal = TMath::Mean (window_width, datacluster->data_out)*dy;
                ...
                for(int i = 0; i < window_length; i++){
                    waveform_voltage[i] += getVoltage(window_length, datacluster, pedestal, dy, i);  
                }
        ...
        TH1F* avg_wfm = new TH1F("avg_wfvm","",length_trace,0,length_trace*0.1);
        for(int i = 0; i < length_trace; i++){
            avg_wfm->SetBinContent(i+1, waveform_voltage[i]/trace_count);
        }
...
void chargeIntegral(TH1F* charges_signal){
    ...
    TString charge_int = Form("Integral %.2f", integral);
    TLatex *myt = new TLatex(0,0,charge_int);
    list->Add(myt);
